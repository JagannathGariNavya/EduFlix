...
export function Users() {
  ...
  useEffect(() => {
    // Fetch user data
    axios
      .get("https://eduschool-2.onrender.com/user")
      .then((response) => {
  ...
  const handleDelete = (id) => {
    axios
      .delete(`https://eduschool-2.onrender.com/user/${id}`)
      .then(() => {
  ...
  const handleSave = () => {
    axios
      .put(`https://eduschool-2.onrender.com/user/${editId}`, editData)
      .then((response) => {
        setDatas((prevData) =>
  ...
  return (
    <main className={`main-container ${isOpen ? "blur-background" : ""}`}>
      ...
      <div className="main-cards">
        ...
        <div className="card" style={{ backgroundColor: "#ff2a00" }}>
          <div className="card-inner" >
      ...
      <div style={{ textAlign: "center", marginTop: "20px" }}>
        ...
        <button
          className="pagination_button"
          onClick={() => paginate(currentPage + 1)}
          disabled={indexOfLastItem >= datas.length}
        >
          Next
        </button>
      </div>
